...
test: build
    ACT="{{act}}" uv run --project e2e pytest e2e/ -v --ignore=e2e/test_agent_mode.py

# Agent mode e2e: creates wallet + policy + API key under $HOME/.ows, then
# tests policy enforcement. Not part of `test`/CI — needs the real `ows` CLI
# and a real, un-sandboxed $HOME/.ows (the component reads API keys/policies
# from a hard-coded /ows path; see e2e/test_agent_mode.py's module docstring).
test-agent: build
    ACT="{{act}}" OWS="{{ows}}" uv run --project e2e pytest e2e/test_agent_mode.py -v
